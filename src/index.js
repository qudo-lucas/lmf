const { mkdirp, copy, remove, writeFile, readFile, pathExists } = require('fs-extra');
...
const loadConfig = async () => {
    try {
        const exists = await pathExists("lmf.config.js");
        if(!exists) {
            return {};
        }

        return JSON.parse(await readFile("lmf.config.js"));
    } catch(err) {
        console.log("[ERROR] LMF : Failed to parse lmf.config.js");
        throw(err);
    }
}
